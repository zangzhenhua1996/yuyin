...
class DataSpeech():
    ...
    def __init__(self, path, type, LoadToMem = False, MemWavCount = 10000):
        ...
        self.dic_wavlist_thchs30 = {}  #这个是文件的路径列表
        self.dic_symbollist_thchs30 = {} #这个是文件的拼音字符的字典
        self.dic_wavlist_stcmds = {}
        self.dic_symbollist_stcmds = {}
        
        self.SymbolNum = 0 # 记录拼音符号数量
        self.list_symbol = self.GetSymbolList() # 全部汉语拼音符号列表,一开始就将所有的拼音符号读取加载到这个list_symbol中
        self.list_wavnum=[] # wav文件标记列表
        self.list_symbolnum=[] # symbol标记列表
        
        self.DataNum = 0 # 记录数据量
        self.LoadDataList()   #实例化对象的时候就执行加载数据的函数
    ...
    def LoadDataList(self): 
        '''
        加载数据的时候加载的是thchs3以及st-cmds的三种训练集,给定的type是train那么就获取两个数据集的train的所有的数据,数据列表及拼音串
        加载用于计算的数据列表
        参数：
            type：选取的数据集类型
                train 训练集
        ...
        # 设定选取哪一项作为要使用的数据集
        if(self.type=='train'):
            filename_wavlist_thchs30 = 'thchs30' + self.slash + 'train.wav.lst'   #self.slash就是个 \ 或者是 //
    ...
    def GetDataNum(self):
        ...
        '''
        num_wavlist_thchs30 = len(self.dic_wavlist_thchs30)   #这个是thchs_30(train或者是test等)的数据集的个数
        num_symbollist_thchs30 = len(self.dic_symbollist_thchs30)  #symbollist这个就是拼音串
...
if(__name__=='__main__'):
    path='Z:\\dataset'
    l=DataSpeech(path,'dev')
    l.LoadDataList()
    print(l.GetDataNum())  #获取整个数据总量
    a = l.GetSymbolList()
    b = l.GetSymbolNum()
    c = l.list_symbol
    d = l.SymbolToNum('a1')
...
#   print(l.GetData(0))
# =============================================================================
#   aa=l.data_genetator(batch_size=2)
#   aa_list = []
#   for i in aa:
# #     a,b=i
# #     print(a,b)
