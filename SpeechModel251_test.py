    ...
    def TrainModel(self, datapath, epoch = 2, save_step = 1000, batch_size = 32, filename = abspath + 'model_speech/m' + ModelName + '/speech_model'+ModelName):
        ...
        for epoch in range(epoch): # 迭代轮数
            ...
            while True:
                ...
                self.TestModel(self.datapath, str_dataset='train', data_count = 64, out_report = True) #进行训练集模型的测试
                self.TestModel(self.datapath, str_dataset='dev', data_count = 64, out_report = True) # 进行 验证集模型的测试
