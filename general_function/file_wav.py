...
from python_speech_features import mfcc  #获取mfcc特征
from python_speech_features import delta  #mfcc特征向量的变换系数
from python_speech_features import logfbank  #fbank特征

from scipy.fftpack import fft #傅里叶变换
...
def GetMfccFeature(wavsignal, fs): #wavsignal就是信号强度wave_data,fs就是采样频率
    # 获取输入特征
    feat_mfcc=mfcc(wavsignal[0],fs)  #特征向量  
...
#后面使用的时候需要加窗就直接成上 w就行了
x=np.linspace(0, 400 - 1, 400, dtype = np.int64)  #横坐标
w = 0.54 - 0.46 * np.cos(2 * np.pi * (x) / (400 - 1) ) # 汉明窗纵坐标
...
def GetFrequencyFeature3(wavsignal, fs):
    ...
    range0_end = int(len(wavsignal[0])/fs*1000 - time_window) // 10 # 计算循环终止的位置，也就是最终生成的窗数
    data_input = np.zeros((range0_end, 200), dtype = np.float) # 用于存放最终的频率特征数据(200维)
    data_line = np.zeros((1, 400), dtype = np.float)
...
def GetFrequencyFeature4(wavsignal, fs):
    ...
    range0_end = int(len(wavsignal[0])/fs*1000 - time_window) // 10 + 1 # 计算循环终止的位置，也就是最终生成的窗数,,这里的bug
    data_input = np.zeros((range0_end, window_length // 2), dtype = np.float) # 用于存放最终的频率特征数据,这里有问题,以后再看
...
#   wave_data, fs = read_wav_data("A2_0.wav")  
    wave_data, fs = read_wav_data("Z:\\dataset\\data_thchs30\\data\\A2_0.wav")
    wav_show(wave_data[0],fs)
    t0=time.time()
    # freimg = GetMfccFeature(wave_data,fs)
    freimg = GetFrequencyFeature3(wave_data,fs)  #使用4有bug
